...
async function getRequests() {
  const { data, error } = await supabase.from("joinRequests").select();
  ...
  while (true) {
    ...
    for (let i = 0; i < requestList.requesterIDs.length; i++) {
      if (requestList.creationTimes.length > 0) {
        if (
          !(await checkUserID(
            requestList.requesterIDs[i],
            requestList.serverIDs[i]
          ))
        ) {
          await clearRequest(
            requestList.requesterIDs[i],
          ...
          await clearEndorsements(
            requestList.requesterIDs[i],
            requestList.serverIDs[i]
          );
        }
        if (
          Date.now() - new Date(requestList.creationTimes[i]).getTime() >
          1209600000
        ) {
          //check every 2 weeks
          await clearRequest(
    ...
    for (let i = 0; i < items[0].creationDates.length; i++) {
      if (items[0].creationDates.length > 0) {
        ...
        if (
          Date.now() - new Date(items[0].creationDates[i]).getTime() >
          2592000000
        ) {
